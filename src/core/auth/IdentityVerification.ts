...
function classifyIdentityResponse(profile: AuthProfile, response: HttpResponse): IdentityVerificationResult {
  ...
  if (/login|sign in|required to log in/i.test(analysisBody)) return { ...base, category: "LOGIN_PAGE_RESPONSE", notes: ["Identity endpoint returned a login-like response."] };
  ...
  if (config.accountStateField && profile.accountState && !extracted.accountState) return withIdentity(base, extracted, "REQUIRED_METADATA_MISSING", true, "Configured account-state metadata was missing or invalid.");
  ...
  if (config.accountStateField && profile.accountState && extracted.accountState !== profile.accountState) return withIdentity(base, extracted, "ACCOUNT_STATE_MISMATCH", true, "Verified account state did not match declared accountState.");

  const partial = (config.tenantIdField && !extracted.tenantId) || (config.roleField && !extracted.role) || (config.accountStateField && !extracted.accountState);
...
function extractIdentity(config: NonNullable<AuthProfile["identityVerification"]>, source: Record<string, unknown>): ExtractedIdentity {
  ...
  const accountState = config.accountStateField ? scalarAtPath(source, config.accountStateField) : undefined;
...
function withIdentity(
  ...
  return {
    ...
    ...(identity.accountState ? { accountStateHash: hashValue(identity.accountState), accountStateMatched: true } : {}),
...
function declaredOnly(profile: AuthProfile): IdentityVerificationResult {
  return {
    ...
    ...(profile.accountState ? { accountStateHash: hashValue(profile.accountState) } : {}),
