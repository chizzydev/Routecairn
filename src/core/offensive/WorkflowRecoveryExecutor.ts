...
export async function recoverWorkflow(checkpoint: WorkflowCheckpoint, options: Pick<ScanContextOptions, "mutationJournalDir" | "authProfile" | "authProfileSet" | "recoveryScope">): Promise<{ caseId: string; cleanupOutcome: "ROLLBACK_VERIFIED" | "CLEANUP_FAILED"; notes: string[] }> {
  ...
  const executors = { authenticationLifecycle: executeAuthenticationLifecycle, businessInvariant: executeBusinessInvariant, controlledRace: executeControlledRace, linkPortalSecurity: executeLinkPortalSecurity, operationalEndpointSecurity: executeOperationalEndpointSecurity, billingEntitlement: executeBillingEntitlement };
