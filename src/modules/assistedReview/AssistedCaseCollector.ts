...
export function collectAssistedCases(result: ModuleResult, findings: readonly Finding[] = result.findings ?? []): AssistedReviewCase[] {
  ...
  for (const value of result.protocolSecurity?.observations ?? []) add("API", { id: value.caseId, label: value.label, outcome: value.outcome, ...(value.cleanupOutcome ? { cleanup: value.cleanupOutcome } : {}), fingerprint: value.comparisonFingerprint });
...
export function collectReportAssistedCases(report: Partial<RouteCairnReport>): AssistedReviewCase[] {
  const modules = {
    ...
    billingEntitlement: "billing-entitlement-security", secretBoundary: "secret-boundary", activeVulnerability: "active-vulnerability-validation", protocolSecurity: "protocol-security", browserCrawl: "browser-crawler"
  } as const;
