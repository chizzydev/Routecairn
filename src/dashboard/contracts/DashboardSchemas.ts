...
export const dashboardScanCreateSchema: z.ZodType<DashboardScanCreateRequest> = z.object({
  ...
  if ((value.inventoryImport || value.inventoryImportFile) && (value.apiGraphql || value.apiGraphqlFile || value.supabaseAuthorization || value.supabaseAuthorizationFile || value.studio?.workflows.some((workflow) => workflow.enabled && (workflow.workflowId === "object-pair" || workflow.workflowId === "collection-authorization" || workflow.workflowId === "file-authorization")))) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["inventoryImport"], message: "Inventory import cannot be combined with explicit API, Supabase, object-pair, collection, or file workflow inputs." });
