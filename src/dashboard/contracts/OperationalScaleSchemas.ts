...
export const cloudSyncPeerSchema = z.object({ organizationId: z.string().uuid(), remoteOrganizationId: z.string().uuid().optional(), name: safeName, endpoint: httpsUrl, sharedSecretEnv: envName, enabled: z.boolean().default(true), syncMode: z.enum(["FULL_STATE", "SAFE_EVENTS"]).default("FULL_STATE") }).strict();
const cloudStateEntitySchema = z.object({ entityType: z.enum(["project","target","scan","scan-plan","scan-module","scan-event","finding","finding-occurrence","evidence","finding-review","finding-note","finding-remediation","finding-retest","credential","configuration","configuration-version","proof-pack","proof-pack-finding","artifact","artifact-chunk","comparison","comparison-module","comparison-workflow","comparison-case","comparison-finding","membership"]), entityId: z.string().regex(/^(?:[0-9a-f-]{36}|[0-9a-f]{32})$/i), originInstallationId: z.string().uuid(), updatedAt: z.string().datetime(), rowVersion: z.number().int().positive(), payloadDigest: z.string().regex(/^[a-f0-9]{64}$/), data: z.record(z.unknown()) }).strict();
const cloudStateSchema = z.object({ schemaVersion: z.literal(1), generatedAt: z.string().datetime(), sourceInstallationId: z.string().uuid(), stateDigest: z.string().regex(/^[a-f0-9]{64}$/), fullStateDigest:z.string().regex(/^[a-f0-9]{64}$/).optional(),nextCursor:z.string().max(200).optional(), entities: z.array(cloudStateEntitySchema).max(10000) }).strict();
export const cloudSyncPushSchema = z.object({ organizationId: z.string().uuid(), cursor: z.number().int().nonnegative(), events: z.array(z.object({ eventId: z.string().uuid(), entityType: z.string().max(100), entityId: z.string().max(200), operation: z.enum(["UPSERT", "DELETE"]), payload: z.record(z.unknown()), payloadDigest: z.string().regex(/^[a-f0-9]{64}$/), originInstallationId: z.string().uuid(), createdAt: z.string().datetime() }).strict()).max(500), state: cloudStateSchema.optional() }).strict();
export const cloudSyncMembershipBindSchema=z.object({organizationId:z.string().uuid(),originInstallationId:z.string().uuid(),sourceUserId:z.string().uuid(),localUserId:z.string().uuid()}).strict();
