...
export class EvidenceGovernanceService {
  ...
  private safeArtifactPath(value:string):string { const candidate=canonicalPath(value); const roots=[this.paths.reportsDir,this.paths.proofPacksDir,this.paths.artifactsDir,this.paths.integrationsDir].map(canonicalPath); if(!roots.some((root)=>candidate===root||candidate.startsWith(`${root}\\`)||candidate.startsWith(`${root}/`))) throw new Error("EVIDENCE_PATH_OUTSIDE_DASHBOARD_ROOTS"); return candidate; }
