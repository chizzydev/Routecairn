...
function markdown(result: BenchmarkResult): string {
  ...
  const slices = result.slices.map((item) => `| ${item.dimension} | ${escapeMd(item.value)} | ${item.caseCount} | ${pct(item.metrics.recall)} | ${pct(item.metrics.falsePositiveRate)} | ${item.metrics.youdenIndex.toFixed(4)} |`).join("\n");
