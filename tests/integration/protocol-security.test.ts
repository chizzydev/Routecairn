...
describe("protocol security integration", () => {
  it("executes bounded SSE plus an authorized GraphQL mutation and verified cleanup without payload persistence", async () => {
    let mutated = false; let cleanupCount = 0; let binaryUploadObserved = false; let multipartCreated = false; let multipartCleanupCount = 0;
    server = createServer(async (request, response) => {
      ...
      if (request.url === "/inspect-upload" && request.method === "POST") { const raw = await rawBody(request); binaryUploadObserved = raw.indexOf(Buffer.from([0, 255, 1])) >= 0; multipartCreated = binaryUploadObserved; response.writeHead(binaryUploadObserved ? 201 : 400, { "content-type": "application/json" }).end('{"ok":true}'); return; }
      if (request.url === "/uploads/reset" && request.method === "POST") { if (request.headers.authorization !== "Bearer header-private-value") return void response.writeHead(401).end(); multipartCreated = false; multipartCleanupCount += 1; response.writeHead(204).end(); return; }
    ...
    const approval = { environment: "TEST", operator: "operator-private-name", ticket: "ticket-private-123", authorizedAt: new Date(now - 1000).toISOString(), expiresAt: new Date(now + 60_000).toISOString(), confirmation: "I_AUTHORIZE_PROTOCOL_STATE_CHANGES", disposableResources: true } as const;
    const manifest = { schemaVersion: 1, maxRequests: 6, maxDurationMs: 3000, actors: [{ id: "member", safeAlias: "member", authSlot: "primary", relationship: "self" }], cases: [
      ...
      { id: "upload", label: "controlled upload with cleanup", kind: "MULTIPART_UPLOAD", actorId: "member", requireVerifiedIdentity: false, url: `${origin}/inspect-upload`, headers: {}, fields: { purpose: "fixture" }, files: [{ fieldName: "file", fileName: "fixture.bin", contentType: "application/octet-stream", contentSecretRef: "upload_fixture" }], readOnly: false, expectation: { decision: "ALLOW", allowedStatuses: [201], deniedStatuses: [400, 401, 403], minMessages: 0 }, authorization: approval, cleanup: { url: `${origin}/uploads/reset`, method: "POST", headers: {}, body: { fixture: "{{SECRET:fixture_id}}" }, statusIn: [204] } },
      { id: "mutation", label: "controlled mutation", kind: "GRAPHQL_MUTATION", actorId: "member", requireVerifiedIdentity: false, url: `${origin}/graphql`, headers: {}, operationName: "UpdateFixture", document: "mutation UpdateFixture { updateFixture { ok } }", variables: { input: "{{SECRET:mutation_input}}" }, expectation: { decision: "ALLOW", allowedStatuses: [200], deniedStatuses: [400, 401, 403], jsonPath: "data.updateFixture.ok", equals: true, minMessages: 0 }, authorization: approval, cleanup: { url: `${origin}/fixtures/reset`, method: "POST", headers: {}, body: { fixture: "{{SECRET:fixture_id}}" }, statusIn: [204] } }
    ] };
