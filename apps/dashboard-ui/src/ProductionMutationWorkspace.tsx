...
export function ProductionMutationWorkspace() {
  ...
  const guidedCase = useMemo(() => buildCase({ targetId, credentialId, caseId, alias, authorityField, mutationValue, rollbackValue, allowedValues, identity, precondition, mutationPath, mutationMethod, impact, protectedEnabled, protectedAction, rollbackPath, rollbackMethod, restoration, expiresAt, acknowledged }), [targetId, credentialId, caseId, alias, authorityField, mutationValue, rollbackValue, allowedValues, identity, precondition, mutationPath, mutationMethod, impact, protectedEnabled, protectedAction, rollbackPath, rollbackMethod, restoration, expiresAt, acknowledged]);
  ...
  return <section>
    ...
    {mode === "guided" ? <div className="production-mutation-builder form">
      ...
      <fieldset><legend>2. Authority transition</legend><label>Authority field<input value={authorityField} onChange={(event) => edit(setAuthorityField, event.target.value)} /></label><label>Mutation value (JSON scalar)<input value={mutationValue} onChange={(event) => edit(setMutationValue, event.target.value)} /></label><label>Rollback value (JSON scalar)<input value={rollbackValue} onChange={(event) => edit(setRollbackValue, event.target.value)} /></label><label>Explicitly allowed values (comma-separated JSON scalars)<input value={allowedValues} onChange={(event) => edit(setAllowedValues, event.target.value)} /></label><p className="muted">RouteCairn derives bodies as <code>{`{ "${authorityField}": value }`}</code>; additional mutation fields cannot be added.</p></fieldset>
